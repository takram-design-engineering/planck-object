...
import { LineLoop } from '../..'

const { expect } = chai
chai.use(sinonChai)

describe('LineLoop', () => {
  it('is instance of Three.LineLoop', () => {
    const object = new LineLoop()
    expect(object).instanceof(Three.LineLoop)
  })

  describe('#add', () => {
    it('dispatches event with `to` property', () => {
      const parent = new LineLoop()
      const child = new LineLoop()
  ...
  describe('#addedToScene', () => {
    it('is called when added to the scene graph', () => {
      ...
      const parent = new LineLoop()
      const child1 = new LineLoop()
      const child2 = new LineLoop()
